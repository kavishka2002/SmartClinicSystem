...
interface User extends DemoUser {
  role: string;
  isDemo?: boolean;
}
...
const DEMO_USERS: Record<string, DemoUser> = {
  patient: { phone: "0771234567", password: "patient123", name: "Amal Perera", age: 32, gender: "Male", address: "123 Kandy Rd, Colombo" },
  doctor: { username: "dr.silva", password: "doctor123", name: "Dr. Nimal Silva", specialization: "Cardiology", email: "nimal@clinic.lk" },
  staff: { username: "staff", password: "staff123", name: "Front Desk Staff", email: "staff@clinic.test" },
  pharmacy: { username: "pharmacy", password: "pharmacy123", name: "Clinic Pharmacy", email: "pharmacy@example.test" },
};

const SAMPLE_PATIENTS: Patient[] = [
  { id: 1, name: "Amal Perera", age: 32, gender: "Male", phone: "0771234567", status: "Waiting", token: "T-001" },
  { id: 2, name: "Seetha Wijesinghe", age: 45, gender: "Female", phone: "0762345678", status: "With Doctor", token: "T-002" },
  { id: 3, name: "Ruwan Bandara", age: 28, gender: "Male", phone: "0753456789", status: "Done", token: "T-003" },
];

const SAMPLE_APPOINTMENTS: Appointment[] = [];

const SAMPLE_PRESCRIPTIONS: Prescription[] = [
  { id: 1, patient: "Amal Perera", doctor: "Dr. Nimal Silva", medicines: ["Paracetamol 500mg", "Amoxicillin 250mg"], date: "2026-05-13", status: "Pending" },
  { id: 2, patient: "Seetha Wijesinghe", doctor: "Dr. Priya Fernando", medicines: ["Metformin 500mg", "Lisinopril 10mg"], date: "2026-05-12", status: "Dispensed" },
];

const SAMPLE_STOCK: Stock[] = [
  { id: 1, name: "Paracetamol 500mg", qty: 450, unit: "tablets", status: "In Stock" },
  { id: 2, name: "Amoxicillin 250mg", qty: 120, unit: "capsules", status: "In Stock" },
  { id: 3, name: "Metformin 500mg", qty: 18, unit: "tablets", status: "Low Stock" },
  { id: 4, name: "Lisinopril 10mg", qty: 0, unit: "tablets", status: "Out of Stock" },
  { id: 5, name: "Ibuprofen 400mg", qty: 200, unit: "tablets", status: "In Stock" },
];
...
function LoginPageInner() {
  ...
  const [stock, setStock] = useState<Stock[]>(SAMPLE_STOCK);
  ...
  const handleLogin = async () => {
    ...
    // Client-side demo login support (local demo accounts)
    try {
      const demoUser = DEMO_USERS[selectedRole.id];
      if (demoUser) {
        const demoMatch = selectedRole.id === "patient"
          ? loginForm.phone.trim() === (demoUser.phone || "") && password === demoUser.password
          : loginForm.username.trim() === (demoUser.username || "") && password === demoUser.password;
        ...
        if (demoMatch) {
          ...
          notify("Logged into demo account.");
      ...
      // ignore and continue to real login
    }
    ...
    try {
      ...
      if (data.redirectTo) {
        router.push(data.redirectTo);
            ...
            // Use the clinic's main dashboard for staff
            setScreen("dashboard");
            setActiveTab("dashboard");
          } else {
        setScreen("dashboard");
              ...
              <div style={{ background:"rgba(255,255,255,0.06)", border:"1px solid rgba(255,255,255,0.14)", borderRadius:26, padding:34, backdropFilter:"blur(22px)" }}>
                ...
                <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
                  ...
                  <div style={{ textAlign:"center", color:"#64748b", fontSize:12, marginTop:8 }}>
                    ...
                    ) : selectedRole?.id === "pharmacy" ? (
                      "Pharmacy demo: username 'pharmacy' / password 'pharmacy123'"
                    ) : (
                      "Use a registered system account for admin, staff, or pharmacy login."
                    )}
                  </div>
                </div>
        ...
        <div style={{ minHeight:"100vh", display:"flex", flexDirection:"column" }}>
          ...
          <div style={{ display:"flex", flex:1 }}>
            ...
            <div style={{ flex:1, padding:28, overflowY:"auto" }}>
              ...
              {/* STAFF DASHBOARD */}
              {user.role==="staff" && (
                <StaffDashboard user={user} activeTab={activeTab} doctors={doctors} patients={patients} setPatients={setPatients} appointments={appointments} setAppointments={setAppointments} newPatientForm={newPatientForm} setNewPatientForm={setNewPatientForm} notify={notify} />
              )}
              {/* PHARMACY DASHBOARD */}
              {user.role==="pharmacy" && (
...
function getNavItems(role: string) {
  const maps: Record<string, { id: string; icon: string; label: string }[]> = {
    ...
    staff: [{id:"dashboard",icon:"📊",label:"Dashboard"},{id:"register",icon:"➕",label:"Register Patient"},{id:"appointments",icon:"📅",label:"Appointments"},{id:"queue",icon:"🔢",label:"Queue System"}],
...
function StaffDashboard({ user, activeTab, doctors, patients, setPatients, appointments, setAppointments, newPatientForm, setNewPatientForm, notify }: { user: User | null; activeTab: string; doctors: Doctor[]; patients: Patient[]; setPatients: React.Dispatch<React.SetStateAction<Patient[]>>; appointments: Appointment[]; setAppointments: React.Dispatch<React.SetStateAction<Appointment[]>>; newPatientForm: { name: string; phone: string; age: string; gender: string; address: string; password: string; confirmPassword: string }; setNewPatientForm: React.Dispatch<React.SetStateAction<{ name: string; phone: string; age: string; gender: string; address: string; password: string; confirmPassword: string }>>; notify: (msg: string, type?: string) => void }) {
  ...
  const saveDoctorEdit = async (doc: Doctor) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // API failed (likely demo/no-session). Apply local update so demo users can see changes immediately.
        const updatedLocal = doctors.map(d => d.id === doc.id ? { ...d, paymentRequired: !!editingDoctorPaymentRequired, fee: normalizedFee, doctorCharges: normalizedFee } : d);
        window.dispatchEvent(new CustomEvent('doctors-updated', { detail: updatedLocal }));
        notify(data?.message || 'Updated locally (no server session).', res.status === 401 ? 'warning' : 'info');
        setEditingDoctorId(null);
      }
    } catch (err) {
  ...
  if (activeTab==="appointments") return (
    <div style={{ animation:"fadeIn 0.4s ease" }}>
      <SectionTitle title="Manage Appointments" sub="View and update appointment status" />
      <div style={{ display:"grid", gap:14 }}>
        {appointments.length === 0 && <div style={{ color:"#94a3b8" }}>No appointments found.</div>}
        {appointments.map(a=>(
          <div key={a.id} style={{ background:"rgba(255,255,255,0.04)", border:"1px solid rgba(255,255,255,0.08)", borderRadius:16, padding:18 }}>
            <div style={{ display:"flex", justifyContent:"space-between", gap:14, flexWrap:"wrap", marginBottom:12 }}>
              <div style={{ flex:1, minWidth:0 }}>
                <div style={{ color:"#fff", fontSize:15, fontWeight:600 }}>{a.patient}</div>
                <div style={{ color:"#64748b", fontSize:12 }}>🩺 {a.doctor} · 📅 {a.date} · 🕐 {a.time}</div>
                {a.patientContact && <div style={{ color:"#94a3b8", fontSize:12, marginTop:6 }}>Contact: {a.patientContact}</div>}
              </div>
            ...
            <div style={{ display:"flex", gap:10, flexWrap:"wrap" }}>
              {a.status !== "Confirmed" && (
                <button onClick={() => updateAppointmentStatus(a.id, { status: "Confirmed" })} disabled={updatingAppointmentId === String(a.id)} style={{ background:"rgba(16,185,129,0.15)", border:"1px solid rgba(16,185,129,0.3)", color:"#34d399", padding:"8px 14px", borderRadius:8, cursor:"pointer", fontSize:12, fontWeight:600 }}>
                  {updatingAppointmentId === String(a.id) ? "Updating..." : "Confirm"}
                </button>
              )}
              {a.status !== "Cancelled" && (
                <button onClick={() => updateAppointmentStatus(a.id, { status: "Cancelled" })} disabled={updatingAppointmentId === String(a.id)} style={{ background:"rgba(239,68,68,0.12)", border:"1px solid rgba(239,68,68,0.3)", color:"#f87171", padding:"8px 14px", borderRadius:8, cursor:"pointer", fontSize:12, fontWeight:600 }}>
                  {updatingAppointmentId === String(a.id) ? "Updating..." : "Cancel"}
                </button>
              )}
              {a.paymentRequired && !a.paid && (
                <button onClick={() => updateAppointmentStatus(a.id, { paid: true, status: "Confirmed" })} disabled={updatingAppointmentId === String(a.id)} style={{ background:"rgba(59,130,246,0.15)", border:"1px solid rgba(59,130,246,0.3)", color:"#60a5fa", padding:"8px 14px", borderRadius:8, cursor:"pointer", fontSize:12, fontWeight:600 }}>
                  {updatingAppointmentId === String(a.id) ? "Updating..." : "Mark Paid"}
                </button>
              )}
            </div>
          </div>
  ...
  if (activeTab==="queue") return (
    <div style={{ animation:"fadeIn 0.4s ease" }}>
      <SectionTitle title="Queue System" sub="Manage patient flow" />
      <div style={{ display:"grid", gap:14 }}>
        {appointments.length === 0 && <div style={{ color: "#94a3b8" }}>No queued appointments found yet.</div>}
        {appointments
          .filter((appt) => {
            const status = appt.status.toLowerCase();
            return status === "waiting" || status === "pending" || status === "confirmed" || status === "scheduled" || status === "booked" || status === "in consultation" || status === "with doctor" || status === "ongoing" || status === "started" || status === "under consultation";
          })
          .map((appt) => (
            <div key={appt.id} style={{ background:"rgba(255,255,255,0.04)", border:"1px solid rgba(255,255,255,0.08)", borderRadius:16, padding:18, display:"flex", alignItems:"center", gap:14, flexWrap:"wrap" }}>
              <div style={{ width:44, height:44, borderRadius:12, background:"rgba(139,92,246,0.2)", display:"flex", alignItems:"center", justifyContent:"center", color:"#8b5cf6", fontWeight:800, fontSize:13 }}>{appt.token || `T-${String(appt.queueNumber || 0).padStart(3, "0")}`}</div>
              <div style={{ flex:1, minWidth:0 }}>
                <div style={{ color:"#fff", fontSize:15, fontWeight:600 }}>{appt.patient || appt.patientName || "Unknown patient"}</div>
                <div style={{ color:"#64748b", fontSize:12 }}>Doctor: {appt.doctor || "Unassigned"} · {appt.date} · {appt.time}</div>
                {appt.patientContact && <div style={{ color:"#94a3b8", fontSize:12, marginTop:6 }}>Contact: {appt.patientContact}</div>}
              </div>
              <Badge text={appt.status || "Pending"} type={appt.status === "Confirmed" ? "success" : "warning"} />
              <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
                {(appt.status || "").toLowerCase() !== "with doctor" && (appt.status || "").toLowerCase() !== "in consultation" && (
                  <button onClick={() => updateAppointmentStatus(appt.id, { status: "With Doctor" })} style={{ background:"rgba(14,165,233,0.15)", border:"1px solid rgba(14,165,233,0.3)", color:"#38bdf8", padding:"6px 12px", borderRadius:8, cursor:"pointer", fontSize:12 }}>
                    Call
                  </button>
                )}
                {(appt.status || "").toLowerCase() !== "completed" && (appt.status || "").toLowerCase() !== "done" && (
                  <button onClick={() => updateAppointmentStatus(appt.id, { status: "Completed" })} style={{ background:"rgba(16,185,129,0.15)", border:"1px solid rgba(16,185,129,0.3)", color:"#34d399", padding:"6px 12px", borderRadius:8, cursor:"pointer", fontSize:12 }}>
                    Complete
                  </button>
                )}
              </div>
