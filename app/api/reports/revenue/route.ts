...
import { getRevenueReport, generatePdfReport, generateExcelReport, ReportParams } from "../../../lib/reportService";
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (exportType === "pdf") {
      const pdfBuffer = await generatePdfReport("Revenue", report);
    ...
    if (exportType === "excel") {
      const excelBuffer = await generateExcelReport("Revenue", report);
    ...
    return NextResponse.json({ success: false, message: error instanceof Error ? error.message : "Unable to generate revenue report." }, { status: 500 });
  }
}
